...
#ifndef SMPCMD_SMPCMD_HPP
#define SMPCMD_SMPCMD_HPP
...
class CommandNode {
private:
    std::map<std::string, CommandNode<S>> children;
    std::string literal;
...
public:
    ...
    CommandNode<S> &then(CommandNode<S> child) {
        children[child.literal] = child;
        hasChild = true;
        return *this;
    };

    CommandNode<S> &asLiteral(const std::string& string){
        this->literal = string;
        return *this;
    }

    CommandNode<S> &executes(std::function<uint32_t(CommandContext<S>)> func) {
        this->function = func;
        hasChild = false;
        return *this;
    }
    ...
    CommandNode<S> &asArgument(ArgType argT, const std::string &argName) {
        this->literal = "<" + argName + ">";
        this->argType = argT;
        return *this;
    }

    std::map<std::string, CommandNode<S>> &getChildren() {
        return children;
    }
...
private:
    std::map<std::string, CommandNode<S>> roots;
public:
    static const int commandError = -114514;
private:
...
public:
    ...
    void addChild(CommandNode<S> &nod) {
        roots[nod.getLiteral()] = nod;
    }
private:
    int execute(CommandNode<S> node, const std::vector<std::string> &command, CommandContext<S> commandContext) {
